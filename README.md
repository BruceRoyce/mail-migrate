...
After changing exclusions, destination overrides or the label acknowledgement, click **Discover folders & build plan** again to apply the policy and refresh counts. You do not need to retest connections. The previous plan's approval and download are disabled until the new plan completes; review it and explicitly approve again. Excluded folders remain visible as `explicit_exclusion`, with no message scan or copy. A detected label/aggregate view requires the explicit physical-copy acknowledgement under Folder policy. Loading a saved migration requires matching policy settings as well as matching accounts.
